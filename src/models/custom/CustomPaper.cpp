...
void CustomPaper::makeWaterStroke() {
...
void CustomPaper::addWaterDrop(size_t x0, size_t y0, size_t r) {
    ...
    for (size_t x = 0; x < W; x++) {
        for (size_t y = 0; y < H; y++) {
            float w =
                    sqrt((float) (r * r - ((float) x - x0) * ((float) x - x0) - ((float) y - y0) * ((float) y - y0))) -
                    (float) r / 2.0f;
            if (w > 0) {
...
void CustomPaper::makeInkStroke() {
...
void CustomPaper::setPaperPlane(size_t newB, size_t newC) {
    for (size_t x = 0; x < W; x++)
        for (size_t y = 0; y < H; y++) {
...
void CustomPaper::setHydrophobic() {
    PerlinNoise pn;
    for (size_t i = 0; i < W * H; i++) {
        float x = (float) (i % W) / ((float) W);
        float y = (float) (i / W) / ((float) H);
...
void CustomPaper::addFibres(const PAPER paper) {
...
void CustomPaper::run(size_t cores, size_t cycles) {
    for (size_t n = 0; n < cycles; n++) {
        CustomPaper::WorldUnit::doCalc(cores, this);
    }
}
